...
def remove_undetected(directory ,detector ='hog'):
  ...
  '''
  Removes the undetected images in data folder
...
def face_detector(img,detector = 'hog'):
  ...
  '''
  Detects faces of images in data
...
def make_data_array(directory ,paths,img_size ,imgs_per_folder,total_imgs ,detector = 'Hog', check_detect = True):

  '''
  Loads the data on disk to an array to speed up training
  ...
  detector = detector to detect faces in images
